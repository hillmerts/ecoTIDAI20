...
def inform(problem, x):
    ...
    for i in range(problem.max + 1):
        retvec.append(x[i])
    for i in range(problem.max):        
        retdist.append(problem.D[x[i], x[i+1]]/60.0 + 3.3*(problem.tires[x[i]] + problem.tires[x[i+ 1]])*0.5 +problem.inclustdis[x[i]]*3 )
    return retvec, retdist
...
def routeopt(maxtime,tires,xvec,yvec,interdist):
    ...
    while len(xvecn) > 5:
        ...
        route, dist = inform(problem, res.X)
...
#routeopt(350,tires,xvec,yvec,interdist)
